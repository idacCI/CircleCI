...
class VerificationClassTest extends PHPUnit_Framework_TestCase {
	...
	function 余剰計算の確認() {
		...
		$result = $this->VerificationClass->division();
		...
		$result = $this->VerificationClass->division(10, 5);
		$this->assertTrue($result);

		// 第一引数が第二引数で割り切れない場合はtureが返却されてくる設計
		$result = $this->VerificationClass->division(3, 5);
		$this->assertFalse($result);
	}
	...
	function fizzbuzz出来るかの確認() {
		...
		$result = $this->fizzbuzz('TEST');
		$this->assertFalse($result);

		$result = $this->fizzbuzz(100);
